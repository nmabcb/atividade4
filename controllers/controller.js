...
exports.findall = (req, res) => {
    ...
    async function dataGet() {
        ...
        const newspapers = ['BBC News', 'TheTimes', 'TheGuardian']
        const newspapersUrl = ['https://www.bbc.co.uk/search?q=Nasa', 'https://www.thetimes.co.uk/search?source=nav-desktop&q=Nasa', 'https://www.theguardian.com/science/nasa']
        const base = ['', 'https://www.thetimes.co.uk', '']
        id = 0;
        ...
        for (const newspaperUrl of newspapersUrl) {
            await axios.get(newspaperUrl)
                .then((response) => {
                    ...
                    $('a:contains("Nasa")', html).each(function () {
...
exports.findbbc = (req, res) => {
    const articles = []

    databbc()

    async function databbc() {
        await axios.get("https://www.bbc.co.uk/search?q=Nasa")
            .then(response => {
                const html = response.data
                const $ = cheerio.load(html)
...
exports.findthetimes = (req, res) => {
    const articles = []

    dataTheTimes()

    async function dataTheTimes() {
        await axios.get("https://www.thetimes.co.uk/search?source=nav-desktop&q=Nasa")
            .then(response => {
                const html = response.data
                const $ = cheerio.load(html)
...
exports.findtheguardian = (req, res) => {
    const articles = []

    dataTheGuardian()

    async function dataTheGuardian() {
        await axios.get("https://www.theguardian.com/science/nasa")
            .then(response => {
                const html = response.data
                const $ = cheerio.load(html)
